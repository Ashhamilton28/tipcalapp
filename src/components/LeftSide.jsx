  ...
  return (
    ...
    <div className="mt-6 ">
      ...
      <div className="flex items-center flex-wrap gap-4 mt-4">
        ...
        {buttons.map((label) => (
          <button className="bg-Very-dark-cyan h-10 min-w-[150px] text-white rounded-md">
            {label}
          </button>
        ))}
      </div>
    ...
    <div className="mt-6 relative">
      <label className="text-xl text-Dark-grayish-cyan">
        Number of People
      </label>
      <img className="w-4 absolute top-10 left-2" src={person} alt="" />
      <input
        className="bg-Very-light-grayish-cyan shadow-lg py-2 px-4 w-full text-right"
        type="text"
      />
    </div>
  </div>
